  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('externalUser.export_data_application_form') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('externalUser.export_data_application_form') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { etApplicationListApi } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.globalTrans.year, label_bn: i18n.messages.bn.globalTrans.year, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '10%' } },
...
export default {
  ...
  methods: {
    editData (item) {
      this.$router.push(`/export-trophy-cip-panel/export-trophy-application-form?id=${item.id}`)
    },
    searchData () {
      this.loadData()
    },
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, etApplicationListApi, params)
      if (result.success) {

  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('externalUser.cip_award_invitation') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('externalUser.cip_award_invitation') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(title)="data">
                    <span>{{ (currentLocale == 'bn') ? data.item.invitation_card_cip.title_bn : data.item.invitation_card_cip.title_en }}</span>
                  </template>
                  <template v-slot:cell(year)="data">
                    <span>{{ $n(data.item.invitation_card_cip.year, { useGrouping: false }) }}</span>
                  </template>
                  <template v-slot:cell(venue)="data">
                    <span>{{ (currentLocale == 'bn') ? data.item.invitation_card_cip.title_bn : data.item.invitation_card_cip.title_en }}</span>
                  </template>
                  <template v-slot:cell(date)="data">
                    <span>{{ data.item.invitation_card_cip.date_time | dateFormat }}</span>
                  </template>
...
import { cipPanelInvitationCardList } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.eventManage.year, label_bn: i18n.messages.bn.eventManage.year, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '20%' } },
...
export default {
  ...
  computed: {
    ...
    detailsTitle () {
      return this.$t('externalUser.cip_award_invitation') + ' ' + this.$t('globalTrans.details')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, cipPanelInvitationCardList, params)
      if (result.success) {

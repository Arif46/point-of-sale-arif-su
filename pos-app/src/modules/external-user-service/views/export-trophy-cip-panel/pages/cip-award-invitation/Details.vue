    ...
    <div>
      ...
      <b-card>
        <b-overlay>
          ...
          <b-row>
              <b-col sm="12" style="text-transform:capitalize"><h4 class="text-center mt-3"> {{ $t('eventManage.venue')}} : {{ getVenueNameList(detailsData.venue_id) }}</h4> </b-col>
          </b-row>
        </b-overlay>
      </b-card>
    </div>
...
export default {
  ...
  created () {
    if (this.id) {
      const tmp = this.item.invitation_card_cip
  ...
  methods: {
    pdfExport () {
      const reportTitle = this.$t('externalUser.cip_award_invitation') + ' ' + this.$t('globalTrans.details')
      ExportPdf.exportPdfDetails(exportTrophyCIPServiceBaseUrl, '/configuration/report-heading/detail', 2, reportTitle, this, this.detailsData, this.getVenueNameList, this.formatted)
    },

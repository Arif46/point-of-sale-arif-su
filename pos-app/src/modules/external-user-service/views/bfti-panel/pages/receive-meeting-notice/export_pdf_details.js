...
const exportPdfDetails = async (baseUrl, uri, orgId, reportTitle, vm, data, date, meetingDate) => {
    try {
      ...
      const pdfContent = [{
        ...
        pdfContent.push({ text: data.memo_no + '                                                   Date- ' + date, style: 'fertilizer' })
        pdfContent.push({ text: '', style: 'fertilizer' })
        pdfContent.push({ text: data.meeting_title_en, style: 'fertilizer', alignment: 'center' })
        pdfContent.push({ text: meetingDate, style: 'fertilizer', alignment: 'center' })
        pdfContent.push({ text: '', style: 'fertilizer' })
        pdfContent.push({ text: data.agenda_en, style: 'fertilizer' })
        ...
        data.heads.forEach((head, index) => {
          head.details.forEach((detail, index1) => {
            if (index1 === 0) {
              allRows.push(
                [
                  { text: head.head_title_en, style: 'td', alignment: 'left', bold: true, colSpan: 4 },
                  {},
                  {},
                  {},
                  {}
            ...
            allRows.push(
            [
              { text: detail.item, style: 'td', alignment: 'left' },
              { text: detail.decision, style: 'td', alignment: 'left' },
              { text: detail.update, style: 'td', alignment: 'left' },
              { text: vm.getDesignationList(detail.designation_id), style: 'td', alignment: 'left' },
              { text: vm.getEmployeeName(detail.employee_user_id), style: 'td', alignment: 'left' }
            ]
            )
          })
        })

  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">Model Search</h4>
      </template>
...
import { modelList, modelToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: 'Model Name', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? 'Model Entry' : 'Model Modify'
    },
    pageOptions () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, apiBaseUrl, modelToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(apiBaseUrl, modelList, params)
      if (result.success) {

  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">Brand Search</h4>
      </template>
...
import { brandList, brandToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: 'Brand Name', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? 'Brand Entry' : 'Brand Modify'
    },
    pageOptions () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, apiBaseUrl, brandToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(apiBaseUrl, brandList, params)
      if (result.success) {

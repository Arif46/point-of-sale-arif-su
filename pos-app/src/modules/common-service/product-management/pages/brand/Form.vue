    ...
    <b-overlay :show="loading">
      <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off">
        <b-row>
          ...
          <b-col xs="12" sm="12" md="12" lg="12" xl="12">
            <ValidationProvider name="Brand Name" vid="name" rules="required" v-slot="{ errors }">
              <b-form-group
                label-for="name">
                <template v-slot:label>
...
export default {
  ...
  methods: {
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(apiBaseUrl, `${brandUpdate}/${this.id}`, this.formData)
      } else {
        result = await RestApi.postData(apiBaseUrl, brandStore, this.formData)
      }

  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('orgProfile.upazila') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('orgProfile.upazila_list') }}</h4>
      </template>
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.upazila, label_bn: i18n.messages.bn.orgProfile.upazila, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '35%' } },
  { label_en: i18n.messages.en.orgProfile.district, label_bn: i18n.messages.bn.orgProfile.district, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '35%' } },
  { label_en: i18n.messages.en.globalTrans.status, label_bn: i18n.messages.bn.globalTrans.status, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4 },
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, upazilaList, params)
      if (result.success) {
        const listData = result.data.data.map((item, index) => {
          const district = this.$store.state.CommonService.commonObj.districtList.find(obj => obj.value === parseInt(item.district_id))
            const customItem = {
              district_name: district?.text_en,
              district_name_bn: district?.text_bn
            }
          return Object.assign({}, item, { serial: index + 1 }, customItem)
        })
    ...
    remove (item) {
      this.changeStatus(1, commonServiceBaseUrl, upazilaToggle, item)
    }
  }
}
</script>

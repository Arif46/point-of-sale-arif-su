...
import { categoryList, categoryToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: 'Category Name', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? 'Category Entry' : 'Category Modify'
    },
    pageOptions () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, apiBaseUrl, categoryToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(apiBaseUrl, categoryList, params)
      if (result.success) {

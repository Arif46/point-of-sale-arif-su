  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">Branch Search</h4>
      </template>
...
import { branchList, branchToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.bank_name, label_bn: i18n.messages.bn.orgProfile.bank_name_bn, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 2 },
  { label_en: i18n.messages.en.orgProfile.branch_name, label_bn: i18n.messages.bn.orgProfile.branch_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
  { label_en: i18n.messages.en.orgProfile.address, label_bn: i18n.messages.bn.orgProfile.address_bn, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
  { label_en: i18n.messages.en.orgProfile.mobile, label_bn: i18n.messages.bn.orgProfile.mobile_bn, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? 'Branch Entry' : 'Branch Modify'
    },
    pageOptions () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, apiBaseUrl, branchToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(apiBaseUrl, branchList, params)
      if (result.success) {
        ...
        const bankData = {}
            if (typeof bankObj !== 'undefined') {
              bankData.bank_name = bankObj.text
            } else {
              bankData.bank_name = ''
            }
          return Object.assign({}, item, { serial: index }, bankData)
        })

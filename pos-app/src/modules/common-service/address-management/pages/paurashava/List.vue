  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('orgProfile.municipality_name') }}</h4>
      </template>
      <template v-slot:searchBody>
        <b-row>
          ...
          <b-col xs="12" sm="12" md="6" lg="6" xl="6">
            ...
            >
              <template v-slot:label>
                {{ $t('orgProfile.municipality_name') }}
              </template>
              <b-form-input
                  id="name"
                  type="text"
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('orgProfile.municipality_list') }}</h4>
      </template>
...
import { pauroshavaList, pauroshavaToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.pauroshoba_name, label_bn: i18n.messages.bn.orgProfile.pauroshoba_name_bn, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('orgProfile.municipality') + ' ' + this.$t('globalTrans.entry') : this.$t('orgProfile.municipality') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, apiBaseUrl, pauroshavaToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(apiBaseUrl, pauroshavaList, params)
      if (result.success) {
        ...
        const upazilaData = {}
            ...
            if (typeof upazilaObj !== 'undefined') {
                upazilaData.upazila_name = upazilaObj.text
            } else {
                upazilaData.upazila_name = ''
            }
          return Object.assign({}, item, { serial: index }, divisionData, districtData, upazilaData)
        })

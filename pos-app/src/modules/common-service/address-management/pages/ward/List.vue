  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">Ward Search</h4>
      </template>
...
import { wardList, wardToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.union_name, label_bn: i18n.messages.bn.orgProfile.union_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
  { label_en: i18n.messages.en.orgProfile.ward_name, label_bn: i18n.messages.bn.orgProfile.ward_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3 },
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, apiBaseUrl, wardToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(apiBaseUrl, wardList, params)
      if (result.success) {
        ...
        const unionData = {}
            ...
            if (typeof unionObj !== 'undefined') {
              unionData.union_name = unionObj.text
            } else {
              unionData.union_name = ''
            }
          return Object.assign({}, item, { serial: index }, divisionData, districtData, upazilaData, unionData)
        })

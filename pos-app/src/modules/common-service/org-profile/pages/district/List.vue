  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('orgProfile.district') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('orgProfile.district_list') }}</h4>
      </template>
...
import { districtList, districtToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.division_name, label_bn: i18n.messages.bn.orgProfile.division_name_bn, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '30%' } },
  { label_en: i18n.messages.en.orgProfile.district_name, label_bn: i18n.messages.bn.orgProfile.district_name_bn, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '30%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('orgProfile.district') + ' ' + this.$t('globalTrans.entry') : this.$t('orgProfile.district') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, districtList, params)
      if (result.success) {
        ...
        const divisionData = {}
          ...
          return Object.assign({}, item, { serial: index }, divisionData)
        })
    ...
    remove (item) {
      this.changeStatus(1, commonServiceBaseUrl, districtToggle, item)
    }
  }
}
</script>

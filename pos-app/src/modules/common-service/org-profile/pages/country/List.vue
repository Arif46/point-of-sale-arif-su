  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('orgProfile.country') }}</h4>
      </template>
          ...
          <b-col sm="4">
            ...
            >
              <template v-slot:label>
                {{ $t('orgProfile.country_name') }}
              </template>
              <b-form-input
                  id="country_name"
                  type="text"
                  v-model="search.country_name"
              ></b-form-input>
            </b-form-group>
          </b-col>
          <b-col sm="4">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('orgProfile.country_list') }}</h4>
      </template>
...
import { countryList, countryToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.country_name, label_bn: i18n.messages.bn.orgProfile.country_name_bn, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '70%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('orgProfile.country') + ' ' + this.$t('globalTrans.entry') : this.$t('orgProfile.country') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, countryList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(1, commonServiceBaseUrl, countryToggle, item)
    }
  }
}
</script>
